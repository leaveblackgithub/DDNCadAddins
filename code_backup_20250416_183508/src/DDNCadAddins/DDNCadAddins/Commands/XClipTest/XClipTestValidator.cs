    ...
    public class XClipTestValidator
    {
        ...
        public Dictionary<ObjectId, BlockState> CaptureBlockStates(Database db, List<ObjectId> blockIds)
        {
            ...
            using (Transaction tr = db.TransactionManager.StartTransaction())
            {
                ...
                {
                    if (blockId == ObjectId.Null || !blockId.IsValid) {continue;}
        ...
        private bool IsColorCorrect(BlockReference currentBlock, BlockState initialState)
        {
            try
            {
                ...
                if (initialState.Color.ColorMethod == Autodesk.AutoCAD.Colors.ColorMethod.ByLayer) {return currentBlock.Color.ColorMethod == Autodesk.AutoCAD.Colors.ColorMethod.ByLayer;}
                    
                // 如果初始颜色为BYBLOCK，则当前块应该保持BYBLOCK
                if (initialState.Color.ColorMethod == Autodesk.AutoCAD.Colors.ColorMethod.ByBlock) {return currentBlock.Color.ColorMethod == Autodesk.AutoCAD.Colors.ColorMethod.ByBlock;}
        ...
        private ObjectId FindParentBlockId(Transaction tr, ObjectId blockId)
        {
            try
            {
                // 获取块参照
                BlockReference blockRef = tr.GetObject(blockId, OpenMode.ForRead) as BlockReference;
                if (blockRef == null) {return ObjectId.Null;}
                ...
                if (ownerId == msId) {return ObjectId.Null;}
