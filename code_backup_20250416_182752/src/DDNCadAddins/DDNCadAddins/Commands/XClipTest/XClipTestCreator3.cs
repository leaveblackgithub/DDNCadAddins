...
namespace DDNCadAddins.Commands.XClipTest
{
    ...
    public partial class XClipTestCreator
    {
        ...
        public ObjectId FindNestedBlockReference(Database db, string blockName)
        {
            ...
            using (Transaction tr = db.TransactionManager.StartTransaction())
            {
                ...
                foreach (ObjectId btrId in bt)
                {
                    ...
                    if (blockRefId != ObjectId.Null) {break;}
                }
        ...
        {
            // 防止递归过深
            if (currentLevel >= maxLevel) {return ObjectId.Null;}
                
            BlockTableRecord blockDef = tr.GetObject(blockRef.BlockTableRecord, OpenMode.ForRead) as BlockTableRecord;
            
            // 检查当前块名
            if (blockDef.Name == targetName) {return blockRef.ObjectId;}
            ...
            foreach (ObjectId entId in blockDef)
            {
                ...
                if (ent is BlockReference)
                {
                    ...
                    if (matchId != ObjectId.Null) {return matchId;}
                }
            }
        ...
        public TestData CreateTestEnvironment()
        {
            ...
            try
            {
                ...
                if (byLayerAutoId != ObjectId.Null) {nestedBlockIds.Add("BYLAYER测试块", byLayerAutoId);}
                
                ObjectId byBlockGreenAutoId = FindNestedBlockReference(db, "DDNTest_ByBlock_Green_Auto");
                if (byBlockGreenAutoId != ObjectId.Null) {nestedBlockIds.Add("BYBLOCK测试块", byBlockGreenAutoId);}
                
                ObjectId multiLayerAutoId = FindNestedBlockReference(db, "DDNTest_MultiLayer_Auto");
                if (multiLayerAutoId != ObjectId.Null) {nestedBlockIds.Add("多图层测试块", multiLayerAutoId);}
                
                ObjectId lineTypeAutoId = FindNestedBlockReference(db, "DDNTest_Linetype_Auto");
                if (lineTypeAutoId != ObjectId.Null) {nestedBlockIds.Add("线型测试块", lineTypeAutoId);}
                
                ObjectId deepNestedId = FindDeepNestedBlockReference(db, "DDNTest_D_Auto");
                if (deepNestedId != ObjectId.Null) {nestedBlockIds.Add("深度嵌套块", deepNestedId);}
                ...
                using (Transaction tr = db.TransactionManager.StartTransaction())
                {
                    ...
                    foreach (ObjectId entId in modelSpace)
                    {
                        ...
                        if (ent is BlockReference)
                        {
                            ...
                            if (btr.Name.StartsWith("DDNTest_", StringComparison.Ordinal))
                            {
                    ...
                    {
                        BlockTableRecord btr = tr.GetObject(btrId, OpenMode.ForRead) as BlockTableRecord;
                        if (btr.Name.StartsWith("DDNTest_", StringComparison.Ordinal) && !btr.IsLayout)
                        {
                    ...
                    foreach (ObjectId ltrId in lt)
                    {
                        ...
                        if (ltr.Name.StartsWith("DDNTest_", StringComparison.Ordinal))
                        {
