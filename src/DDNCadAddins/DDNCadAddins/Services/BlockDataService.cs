...
namespace DDNCadAddins.Services
{
    ...
    public class BlockDataService : IBlockDataService
    {
        ...
        {
            if (blockIds == null)
                return OperationResult<(Dictionary<ObjectId, Dictionary<string, string>>, HashSet<string>)>.ErrorResult("图块ID集合为空", TimeSpan.Zero);
                
            if (transaction == null)
                return OperationResult<(Dictionary<ObjectId, Dictionary<string, string>>, HashSet<string>)>.ErrorResult("事务对象为空", TimeSpan.Zero);
