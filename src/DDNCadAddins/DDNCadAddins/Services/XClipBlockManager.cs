...
namespace DDNCadAddins.Services
{
    ...
    public class XClipBlockManager : IXClipBlockManager
    {
        ...
        private readonly IXClipBlockCreator _blockCreator;
        private readonly IXClipBlockFinder _blockFinder;
            ...
            ILogger logger = null)
        {
            ...
            _blockCreator = blockCreator ?? throw new ArgumentNullException(nameof(blockCreator));
        ...
        public OperationResult IsolateXClippedBlocks(Database database, List<XClippedBlockInfo> xclippedBlocks)
        {
            ...
            var result = _acadService.ExecuteInTransaction<bool>(database, tr => {
                // 获取当前文档
                Editor editor = _acadService.GetEditor();
                if (editor == null)
                    throw new Exception("无法获取编辑器");
                    
                // 获取块参照ID列表
                var blockIds = xclippedBlocks.Select(b => b.BlockReferenceId).ToList();
        ...
        public OperationResult AutoXClipAllBlocks(Database database)
        {
            ...
            // 获取所有块引用
            var blockRefs = GetAllBlockReferences(database);
            ...
            foreach (var blockRef in blockRefs)
            {
                try
                {
                    var result = _blockCreator.AutoXClipBlock(database, blockRef.ObjectId);
                ...
                {
                    if (!_suppressLogging)
                        _logger.LogError($"自动XClip块失败: {ex.Message}", ex);
                }
            }
        ...
        public OperationResult AutoXClipBlocksByName(Database database, string blockName)
        {
            ...
            // 获取指定名称的块引用
            var blockRefs = GetBlockReferencesByName(database, blockName);
        ...
        public OperationResult<List<XClippedBlockInfo>> FindAllXClippedBlocks(Database database)
        {
            ...
            Editor editor = _acadService.GetEditor();
            if (editor == null)
                return OperationResult<List<XClippedBlockInfo>>.ErrorResult("无法获取编辑器", TimeSpan.Zero);
