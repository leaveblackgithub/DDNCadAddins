...
namespace DDNCadAddins.Services
{
    ...
    public class BlockReferenceService : IBlockReferenceService
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="BlockReferenceService"/> class.
        ///     构造函数.
        /// </summary>
            ...
            OpenMode openMode = OpenMode.ForRead)
        {
            ...
            {
                this.logger.LogError($"获取块参照对象失败: {ex.Message}", ex);
                return null;
            }
        }
        ...
        public Extents3d? GetBlockGeometricExtents(BlockReference blockRef)
        {
            ...
            {
                this.logger.LogError($"获取块几何边界失败: {ex.Message}", ex);
                return null;
            }
        }
        ...
        public (string BlockName, BlockTableRecord BlockDef)? GetBlockInfo(Transaction tr, BlockReference blockRef)
        {
            ...
            try
            {
                ...
                return blockDef == null ? null : ((string BlockName, BlockTableRecord BlockDef)?)(blockDef.Name, blockDef);
            }
            catch (SystemException ex)
            {
                this.logger.LogError($"获取块属性信息失败: {ex.Message}", ex);
                return null;
            }
        }
        ...
        public ObjectId CreateTestBlock(Transaction tr, string blockName, Point3d insertionPoint)
        {
            ...
            try
            {
                ...
                return this.CreateTestBlock(db, tr, blockName, insertionPoint);
            }
            catch (SystemException ex)
            {
                this.logger.LogError($"创建测试块失败: {ex.Message}", ex);
                return ObjectId.Null;
            }
        }
        ...
        public ObjectId[] FindAllBlockReferences(Editor editor)
        {
            ...
            try
            {
                ...
                return selResult.Status == PromptStatus.OK ? selResult.Value.GetObjectIds() : Array.Empty<ObjectId>();
            }
            catch (SystemException ex)
            {
                this.logger.LogError($"查找块参照失败: {ex.Message}", ex);
                return Array.Empty<ObjectId>();
            }
        }
        ...
        public bool IsBlockXClipped(Transaction tr, BlockReference blockRef, out string detectionMethod)
        {
                    ...
                    if (xdata != null)
                    {
                        foreach (TypedValue value in xdata)
                        {
                            if (value.TypeCode == (int)DxfCode.ExtendedDataRegAppName &&
                                value.Value.ToString().Contains("ACAD_FILTER"))
                            {
            ...
            {
                this.logger.LogError($"检查块是否被XClip失败: {ex.Message}", ex);
                return false;
            }
        }
        ...
        public OperationResult ExecuteXClipCommand(ObjectId blockRefId, Point3d minPoint, Point3d maxPoint)
        {
            ...
            try
            {
                ...
                doc.SendStringToExecute(
                    $"_.XCLIP _New _Rectangular {minPoint.X},{minPoint.Y} {maxPoint.X},{maxPoint.Y} ", true, false,
                    true);
