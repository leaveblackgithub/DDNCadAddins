...
namespace DDNCadAddins.Services
{
    ...
    public class XClipBlockCreator : IXClipBlockCreator
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="XClipBlockCreator"/> class.
        ///     构造函数 - 注入所有依赖项.
        /// </summary>
        ...
        public bool ClipBlockWithRectangle(BlockReference blockRef, Point3d p1, Point3d p2)
        {
            ...
            try
            {
                ...
                // 保存当前选择集状态
                doc.Editor.SetImpliedSelection(new[] { blockRef.ObjectId });
            ...
            catch (Exception ex)
            {
                ...
                {
                    this.logger.LogError("裁剪块失败: " + ex.Message, ex);
                }
        ...
        public OperationResult<ObjectId> CreateTestBlockWithId(Database database)
        {
            ...
            OperationResult<ObjectId> result = this.acadService.ExecuteInTransaction(database, tr =>
            {
                // 创建块定义
            ...
            return !result.Success
                ? OperationResult<ObjectId>.ErrorResult(
                    result.ErrorMessage,
                    DateTime.Now - startTime)
                : new OperationResult<ObjectId>(
                true,
                result.Data,
                "测试块创建成功",
                DateTime.Now - startTime);
        ...
        {
            OperationResult<ObjectId> result = this.CreateTestBlockWithId(database);
        ...
        public OperationResult AutoXClipBlock(Database database, ObjectId blockRefId)
        {
            ...
            OperationResult<bool> result = this.acadService.ExecuteInTransaction(database, tr =>
            {
                ...
                // 检查块是否已被XClip
                if (this.acadService.IsBlockXClipped(tr, blockRef, out string detectionMethod))
                {
                ...
                // 应用裁剪
                return this.ClipBlockWithRectangle(blockRef, minPoint, maxPoint);
            }, "自动XClip块");
