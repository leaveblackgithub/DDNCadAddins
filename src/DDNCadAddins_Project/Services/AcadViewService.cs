...
namespace DDNCadAddins.Services
{
    ...
    public class AcadViewService : IViewService
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="AcadViewService"/> class.
        ///     构造函数.
        /// </summary>
        ...
        public void ZoomToPoint(Document doc, Point3d point, double viewSize)
        {
            ...
            using (ViewTableRecord view = ed.GetCurrentView())
            {
                ...
                this.logger.Log($"已缩放视图到点({point.X:F2}, {point.Y:F2}, {point.Z:F2}), 视图大小: {viewSize}", false);
            }
        }
        ...
        public void ZoomToFirstXClippedBlock(Document doc, List<XClippedBlockInfo> blocks)
        {
            ...
            // 优先选择顶层图块
            List<XClippedBlockInfo> topLevelBlocks = blocks.Where(b => b.NestLevel == 0).ToList();
            ...
            using (Transaction tr = doc.Database.TransactionManager.StartTransaction())
            {
                ...
                if (blockRef != null)
                {
                    this.ZoomToPoint(doc, blockRef.Position, 100); // 使用适当的视图大小
                    this.msgService.ShowMessage($"已缩放到图块 '{targetBlock.BlockName}'");
                }
                else
                {
                    this.msgService.ShowWarning("无法缩放到选定图块：图块不存在或已被删除");
                }
        ...
        public void ProcessAndDisplayXClipResults(List<XClippedBlockInfo> xclippedBlocks)
        {
            ...
            {
                this.msgService.ShowWarning("未找到任何被XClip的图块");
                return;
            }

            // 按照嵌套级别分组显示结果
            List<IGrouping<int, XClippedBlockInfo>> groupedBlocks = xclippedBlocks.GroupBy(b => b.NestLevel)
                .OrderBy(g => g.Key)
                .ToList();

            this.msgService.ShowSuccess($"找到 {xclippedBlocks.Count} 个被XClip的图块");

            foreach (IGrouping<int, XClippedBlockInfo> group in groupedBlocks)
            {
                string nestLevel = group.Key == 0 ? "顶层图块" : $"嵌套级别 {group.Key}";
                this.msgService.ShowMessage($"- {nestLevel}: {group.Count()} 个图块");

                // 在日志中详细记录图块信息，但不显示在命令行
                foreach (XClippedBlockInfo block in group)
                {
                    this.logger.Log($"  图块名称: {block.BlockName}, 检测方法: {block.DetectionMethod}", false);
                }
            }
        }
    }
