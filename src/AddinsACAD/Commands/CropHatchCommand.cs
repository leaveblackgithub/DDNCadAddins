    ...
    public class CropHatchCommand
    {
        ...
        private void ExecuteCropHatch(bool selectAllHatches)
        {
            try
            {
                ...
                // 1b. 从边界曲线 ObjectId 创建 ICropBoundary
                ICropBoundary boundary = null;
                CadServiceManager._.ExecuteInTransactions(null, serviceTrans =>
                {
                    var curve = serviceTrans.GetObject<Curve>(boundaryId, OpenMode.ForRead);
                ...
                // ★ 调用 CropHatchService.ProcessHatches（去除了 Editor 依赖，传入 ICropBoundary）
                var cropHatchService = new CropHatchService(new CropGeometryService());
                var result = cropHatchService.ProcessHatches(
                    hatchIds, boundaryId, boundary, capturedKeepInside);
