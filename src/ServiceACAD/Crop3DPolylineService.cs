...
namespace ServiceACAD
{
    ...
    /// <summary>
    ///     3DPolyline 裁剪服务 — 精确参数搜索 + GetSplitCurves 拆分.
    ...
    public class Crop3DPolylineService
    {
        ...
        private void Process3DPolyline(Polyline3d poly3d, IReadOnlyList<CorePoint2D> bpts, bool keepInside, ITransactionService ts, Crop3DPolylineResult result)
        {
            try
            {
                ...
                var splitParams = FindBoundaryCrossingParams(poly3d, bpts);
                ...
                {
                    splitPts.Add(poly3d.GetPointAtParameter(p));
                }

                DBObjectCollection splitCurves = null;
                try
                {
                    splitCurves = poly3d.GetSplitCurves(splitPts);
                }
                catch (System.Exception ex)
                {
                    Logger._.Warn($"GetSplitCurves 失败 (3DPolyline): {ex.Message}");
                    DeleteEntity(poly3d, result);
                    return;
                }
