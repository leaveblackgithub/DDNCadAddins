    ...
    public class CsvExportService : ICsvExportService
    {
            ...
            string csvFilePath)
        {
            ...
            try
            {
                ...
                if (sortedTags.Contains("Z"))
                {
                    _ = sortedTags.Remove("Z");
                }

                if (sortedTags.Contains("Y"))
                {
                    _ = sortedTags.Remove("Y");
                }

                if (sortedTags.Contains("X"))
                {
                    _ = sortedTags.Remove("X");
                }

                if (sortedTags.Contains("BlockName"))
                {
                    _ = sortedTags.Remove("BlockName");
                }
                ...
                using (StreamWriter sw = new StreamWriter(csvFilePath, false, Encoding.UTF8))
                {
                    ...
                    // 写入每个图块的数据
                    foreach (KeyValuePair<ObjectId, Dictionary<string, string>> blockEntry in blockData)
                    {
                        Dictionary<string, string> values = blockEntry.Value;
