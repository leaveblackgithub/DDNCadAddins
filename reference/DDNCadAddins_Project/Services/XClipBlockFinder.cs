...
namespace DDNCadAddins.Services
{
    ...
    public class XClipBlockFinder : IXClipBlockFinder
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="XClipBlockFinder"/> class.
        ///     构造函数 - 注入所有依赖项.
        /// </summary>
        ...
        public List<ObjectId> FindXClippedBlocks()
        {
            ...
            // 调用完整版方法
            OperationResult<List<XClippedBlockInfo>> result = this.FindXClippedBlocks(db, ed);
            return result.Success && result.Data != null ? result.Data.Select(b => b.BlockReferenceId).ToList() : new List<ObjectId>();
        }
        ...
        public List<ObjectId> FindXClippedBlocksByLayer(string layerName)
        {
            ...
            // 使用事务查询每个块的图层
            OperationResult<List<ObjectId>> filteredResult = this.acadService.ExecuteInTransaction(db, tr =>
            {
                List<ObjectId> layerBlocks = new List<ObjectId>();

                foreach (XClippedBlockInfo blockInfo in result.Data)
                {
            ...
            return filteredResult.Success && filteredResult.Data != null ? filteredResult.Data : new List<ObjectId>();
        }
        ...
        public OperationResult<List<XClippedBlockInfo>> FindXClippedBlocks(Database database, Editor editor)
        {
            ...
            // 使用AcadService执行事务操作
            OperationResult<(List<XClippedBlockInfo> xclippedBlocks, string statusMessage)> transactionResult = this.acadService.ExecuteInTransaction(database, tr =>
            {
                List<XClippedBlockInfo> xclippedBlocks = new List<XClippedBlockInfo>();

                // 获取所有块参照
                ObjectId[] blockRefIds = this.acadService.FindAllBlockReferences(editor);
                ...
                {
                    processed++;
                    this.ProcessBlockReference(tr, id, xclippedBlocks, 0, ref processed, ref skipped);
                }
            ...
            {
                this.logger.LogInfo(transactionResult.Data.statusMessage);
            }
        ...
        public ObjectId FindTestBlock(Transaction tr, Database db, string blockName = "DDNTest")
        {
            ...
            catch (Exception ex)
            {
                if (!this.suppressLogging)
                {
                    this.logger.LogError($"查找测试块时出错: {ex.Message}", ex);
                }
            ...
            List<XClippedBlockInfo> xclippedBlocks, int nestLevel, ref int processed, ref int skipped)
        {
            try
            {
                ...
                // 检查是否被XClip
                bool isXClipped = this.acadService.IsBlockXClipped(tr, blockRef, out string detectionMethod);
                    ...
                    if (btr != null && !btr.IsAnonymous)
                    {
                        ...
                        foreach (ObjectId id in btr)
                        {
                            ...
                            if (ent is BlockReference)
                            {
                                // 递归处理嵌套块
                                processed++;
                                this.ProcessBlockReference(tr, id, xclippedBlocks, nestLevel + 1, ref processed,
                                    ref skipped);
                            }
                        }
                    }
            ...
            catch (Exception ex)
            {
                if (!this.suppressLogging)
                {
                    this.logger.LogError($"处理块参照时出错: {ex.Message}", ex);
                }
