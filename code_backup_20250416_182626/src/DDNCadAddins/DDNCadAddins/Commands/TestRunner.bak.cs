...
";
        ...
        private void RunTestMethod(object instance, MethodInfo method)
        {
            ...
            WriteMessage($"测试 {methodName} {statusText}, 耗时: {result.Duration.TotalSeconds:F2}秒");
        }
        ...
        private void WriteTestSummary()
        {
            ...
            sb.AppendLine($"总耗时: {_context.TotalDuration.TotalSeconds:F2}秒");
            
            // 输出失败的测试
            if (_context.FailedTests > 0)
            {
                sb.AppendLine("\n失败的测试:");
                foreach (var test in _context.Results.Where(t => t.Status == TestStatus.Failed))
                {
        ...
        private string GenerateReport()
        {
            try
            {
                ...
                string reportContent = string.Format(HTML_REPORT_TEMPLATE,
                    ...
                    _context.TotalDuration.TotalSeconds,
        ...
        public void TestXClipService()
        {
            ...
            using (Transaction tr = db.TransactionManager.StartTransaction())
            {
                try
                {
                    ...
                    if (blockId.IsNull)
                        throw new SystemException("Failed to create test block");

                    // 获取块引用
                    BlockReference blockRef = tr.GetObject(blockId, OpenMode.ForWrite) as BlockReference;
                    if (blockRef == null)
                        throw new SystemException("Failed to get block reference");
                    ...
                    if (!success)
                        throw new SystemException("Failed to create XClip boundary");

                    // 验证XClip是否存在
                    if (!VerifyXClipExists(blockRef, tr))
                        throw new SystemException("XClip boundary was not created successfully");
        ...
        public void TestFindXClippedBlocks()
        {
            ...
            if (findResult.Data == null)
                throw new Exception("结果数据不应为空");
            
            // 验证找到的数量
            var foundBlocks = findResult.Data as List<Models.XClippedBlockInfo>;
            if (foundBlocks == null)
                throw new Exception("返回的数据应该是XClippedBlockInfo列表");
            
            // 应该至少找到一个块
            if (foundBlocks.Count == 0)
                throw new Exception("应至少找到一个XClipped块");
            ...
            if (!foundTestBlock)
                throw new Exception("未找到我们创建的XClipped块");
